...
export async function listar(req, res) {
  try {
    const [rows] = await pool.query(
      "SELECT id, nombre, email, rol, sexo, estado, created_at FROM usuarios ORDER BY nombre ASC"
    );
    res.json(rows);
  } catch (err) {
    console.error(err);
...
export async function obtener(req, res) {
  try {
    const [rows] = await pool.query(
      "SELECT id, nombre, email, rol, sexo, estado, created_at FROM usuarios WHERE id = ?",
      [req.params.id]
    );
...
export async function crear(req, res) {
  const { nombre, email, password, rol, sexo } = req.body;
  ...
  try {
    ...
    const [result] = await pool.query(
      "INSERT INTO usuarios (nombre, email, password, rol, sexo) VALUES (?, ?, ?, ?, ?)",
      [nombre, email, hash, rol || "doctor", sexo || null]
    );
...
export async function actualizar(req, res) {
  const { nombre, email, password, rol, estado, sexo } = req.body;
  if (!nombre || !email)
  ...
  try {
    if (password && password.trim() !== "") {
      const hash = await bcrypt.hash(password, 12);
      await pool.query(
        "UPDATE usuarios SET nombre=?, email=?, password=?, rol=?, sexo=?, estado=? WHERE id=?",
        [nombre, email, hash, rol || "doctor", sexo || null, estado ?? 1, req.params.id]
      );
    } else {
      await pool.query(
        "UPDATE usuarios SET nombre=?, email=?, rol=?, sexo=?, estado=? WHERE id=?",
        [nombre, email, rol || "doctor", sexo || null, estado ?? 1, req.params.id]
      );
    }
